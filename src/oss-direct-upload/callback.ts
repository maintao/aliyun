...
const DEFAULT_CALLBACK_BODY =
  "bucket=${bucket}&object=${object}&size=${size}&mimeType=${mimeType}&etag=${etag}";

const defaultPublicKeyCache = new Map<string, string>();
 ...
 * ```
 */
export function encodeOSSDirectUploadCallback(
  config: OSSDirectUploadCallbackConfig,
): OSSDirectUploadCallbackFields {
  const callback: Record<string, string | boolean> = {
    callbackUrl: config.callbackUrl,
    callbackBody: config.callbackBody ?? DEFAULT_CALLBACK_BODY,
    callbackBodyType: config.callbackBodyType ?? "application/x-www-form-urlencoded",
  };
  ...
  const result: OSSDirectUploadCallbackFields = {
...
function isAllowedPubKeyUrl(url: string): boolean {
  return ALLOWED_PUB_KEY_PREFIXES.some((prefix) => url.startsWith(prefix));
}

function getHeader(
  headers: OSSCallbackIncomingRequest["headers"],
  name: string,
): string | undefined {
 ...
 */
export function buildOSSCallbackAuthString(path: string, body: string): string {
...
async function fetchPublicKeyPem(
  ...
  const pem = fetchPublicKey
    ...
    : (await axios.get<string>(url, { responseType: "text" })).data;
 ...
 * ```
 */
export function verifyOSSCallbackSignature(
  publicKeyPem: string,
  authString: string,
 ...
 * ```
 */
export function parseOSSCallbackBody(
  body: string | Buffer,
  ...
  if (contentType.includes("application/json")) {
    return JSON.parse(text) as OSSDirectUploadCallbackPayload;
  }

  const payload: OSSDirectUploadCallbackPayload = {};
...
function fail(status: number, error: string): OSSCallbackHandleResult {
  return {
    ok: false,
    status,
 ...
 * @see https://help.aliyun.com/zh/oss/developer-reference/callback
 */
export async function handleOSSUploadCallback(
  request: OSSCallbackIncomingRequest,
  options: OSSCallbackHandleOptions = {},
): Promise<OSSCallbackHandleResult> {
  ...
  const cache = options.publicKeyCache ?? defaultPublicKeyCache;
  ...
  return {
    ok: true,
    status: 200,
    body: options.responseBody ?? '{"Status":"OK"}',
    payload,
  };
}
