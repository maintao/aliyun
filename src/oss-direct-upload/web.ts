import {
  OSSDirectUploadSignature,
  OSSDirectUploadWebOptions,
  OSSDirectUploadWebResult,
} from "./types";
...
function buildUploadFormData(file: File, signature: OSSDirectUploadSignature, key: string): FormData {
...
): Promise<OSSDirectUploadWebResult> {
 ...
 */
export async function directUploadToOSS(
  options: OSSDirectUploadWebOptions,
): Promise<OSSDirectUploadWebResult> {
  const { file, signature, key: keyOverride, onProgress } = options;
  const key = keyOverride ?? signature.key ?? `${signature.dir}${file.name}`;
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `OSS 直传失败: ${response.status} ${response.statusText}${body ? ` - ${body}` : ""}`,
    );
  }
