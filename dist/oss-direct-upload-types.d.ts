import type { OSSDirectUploadCallbackConfig } from "./oss-direct-upload-callback-types";
/**
...
/** {@link directUploadToOSS} / {@link fetchSignatureAndUploadToOSS} 的返回值 */
...
/**
 * {@link fetchSignatureAndUploadToOSS} 的入参。
 * 封装「向业务服务端取签名 + 直传 OSS」两步。
 *
 * @example
 * ```ts
 ...
 *   signatureFetch: { credentials: "include" },
 * });
 * ```
 */
export interface OSSDirectUploadWithSignatureOptions {
    /** 待上传的文件 */
    file: File;
    /** 业务服务端签发签名的 API 地址（须返回 {@link OSSDirectUploadSignature} JSON） */
    signatureUrl: string;
    /** 获取签名时的 fetch 选项，如 credentials、headers、method */
    signatureFetch?: RequestInit;
    /** 自定义 OSS object key，默认使用 `signature.key` 或 `${dir}${file.name}` */
    key?: string;
    /** 上传进度回调（0-100） */
    onProgress?: (percent: number) => void;
}
//# sourceMappingURL=oss-direct-upload-types.d.ts.map