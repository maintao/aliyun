...
exports.OSSDirectUploadServer = void 0;
const crypto_1 = require("crypto");
const ali_oss_1 = __importDefault(require("ali-oss"));
const oss_direct_upload_callback_1 = require("./oss-direct-upload-callback");
function getStandardRegion(region) {
    return region.replace(/^oss-/i, "");
}
function formatXOssDate(date) {
...
function buildHost(bucket, region, secure) {
    const protocol = secure ? "https" : "http";
    return `${protocol}://${bucket}.oss-${getStandardRegion(region)}.aliyuncs.com`;
}
function buildXOssCredential(accessKeyId, date, region) {
    return `${accessKeyId}/${date}/${getStandardRegion(region)}/oss/aliyun_v4_request`;
}
function hmacSha256(key, data) {
    return Uint8Array.from((0, crypto_1.createHmac)("sha256", key).update(data, "utf8").digest());
}
function signPostObjectPolicyV4(accessKeySecret, region, xOssDate, policyBase64) {
...
class OSSDirectUploadServer {
     ...
     * ```
     */
    async getPostSignature(options) {
        const { dir, key, expireSeconds = 20, // 20 秒有效期
        minSize = 1, maxSize = 10 * 1024 * 1024, // 10MB 最大文件大小
        successActionStatus = "200", callback, } = options;
        const uploadDir = normalizeDir(dir);
        const { region, bucket, accessKeyId, accessKeySecret, roleArn, roleSessionName, secure = true, } = this.config;
        let signingAccessKeyId = accessKeyId;
        let signingAccessKeySecret = accessKeySecret;
        let securityToken;
        if (roleArn) {
            const sts = new ali_oss_1.default.STS({ accessKeyId, accessKeySecret });
            const { credentials } = await sts.assumeRole(roleArn, undefined, expireSeconds, roleSessionName !== null && roleSessionName !== void 0 ? roleSessionName : "oss-direct-upload");
        ...
        const signature = signPostObjectPolicyV4(signingAccessKeySecret, region, xOssDate, policyBase64);
        const result = {
            dir: uploadDir,
        ...
        if (callback) {
            const callbackFields = (0, oss_direct_upload_callback_1.encodeOSSDirectUploadCallback)(callback);
...
exports.OSSDirectUploadServer = OSSDirectUploadServer;
//# sourceMappingURL=oss-direct-upload-server.js.map